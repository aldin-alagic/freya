import React, { useState } from 'react';
import { HorizontalList, HorizontalListItem, HorizontalListItemLabel, HorizontalListItemValue, Thumbnails } from '../../lib/style/generalStyles';
import Thumbnail from '../Thumbnail/Thumbnail';
import {
	ProductCreateReviewWrapper,
	Inner,
	ProductCreateReviewLabel,
	ProductCreateReviewValue,
	ProductCreateReviewRow,
	ProductCreateReviewItems,
	ProductCreateReviewItem,
} from './ProductCreateReviewStyles';

const ProductCreateReview = () => {
	const [selectedIssue, setSelectedIssue] = useState('Body and exterior');
	const product = {
		vehicles: [{ brand: 'BMW', model: '3', year: [2000, 2001], variant: 'M', fuelType: 'Diesel', transmission: 'Manual' }],
		tools: ['Hammer', 'Screwdriver'],
		parts: ['Engine', 'Screws'],
		attachments: [
			{
				name: 'Profile',
				data: 'https://www.pixsy.com/wp-content/uploads/2021/04/ben-sweet-2LowviVHZ-E-unsplash-1.jpeg',
			},
			{
				name: 'Nature',
				data: 'data:image/jpeg;base64,',
			},
			{},
		],
	};

	return (
		<ProductCreateReviewWrapper>
			<Inner>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Vehicle</ProductCreateReviewLabel>
					<ProductCreateReviewValue>
						{product &&
							product.vehicles.map((vehicle) => (
								<HorizontalList isStreched>
									<HorizontalListItem>
										<HorizontalListItemLabel>Brand</HorizontalListItemLabel>
										<HorizontalListItemValue>{vehicle.brand}</HorizontalListItemValue>
									</HorizontalListItem>
									<HorizontalListItem>
										<HorizontalListItemLabel>Model</HorizontalListItemLabel>
										<HorizontalListItemValue>{vehicle.model}</HorizontalListItemValue>
									</HorizontalListItem>
									<HorizontalListItem>
										<HorizontalListItemLabel>Year</HorizontalListItemLabel>
										<HorizontalListItemValue>{vehicle.year.join(', ')}</HorizontalListItemValue>
									</HorizontalListItem>
									<HorizontalListItem>
										<HorizontalListItemLabel>Variant</HorizontalListItemLabel>
										<HorizontalListItemValue>{vehicle.variant}</HorizontalListItemValue>
									</HorizontalListItem>
									<HorizontalListItem>
										<HorizontalListItemLabel>Fuel Type</HorizontalListItemLabel>
										<HorizontalListItemValue>{vehicle.fuelType}</HorizontalListItemValue>
									</HorizontalListItem>
									<HorizontalListItem>
										<HorizontalListItemLabel>Transmission</HorizontalListItemLabel>
										<HorizontalListItemValue>{vehicle.transmission}</HorizontalListItemValue>
									</HorizontalListItem>
								</HorizontalList>
							))}
					</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Issue Description</ProductCreateReviewLabel>
					<ProductCreateReviewValue>
						Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptas sit, obcaecati alias facilis commodi animi quibusdam vel porro aut
						voluptatem nobis temporibus vero possimus laudantium quaerat sequi minus veritatis eaque.
					</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Issue Category</ProductCreateReviewLabel>
					<ProductCreateReviewValue>Additional electric parts</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Issue Type</ProductCreateReviewLabel>
					<ProductCreateReviewValue>Actuator</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Issue Error Code</ProductCreateReviewLabel>
					<ProductCreateReviewValue>XXXXXX</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Issue Attachments</ProductCreateReviewLabel>
					<ProductCreateReviewValue>
						<Thumbnails>
							{product.attachments.map((attachment) => (
								<Thumbnail attachment={attachment} />
							))}
						</Thumbnails>
					</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Solution Title</ProductCreateReviewLabel>
					<ProductCreateReviewValue>Actuator problem solution</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Solution Description</ProductCreateReviewLabel>
					<ProductCreateReviewValue>
						Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptas sit, obcaecati alias facilis commodi animi quibusdam vel porro aut
						voluptatem nobis temporibus vero possimus laudantium quaerat sequi minus veritatis eaque.
					</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Required Tools</ProductCreateReviewLabel>
					<ProductCreateReviewValue>
						<ProductCreateReviewItems>
							{product.tools && product.tools.map((tool) => <ProductCreateReviewItem>{tool}</ProductCreateReviewItem>)}
						</ProductCreateReviewItems>
					</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Required Parts</ProductCreateReviewLabel>
					<ProductCreateReviewValue>
						<ProductCreateReviewItems>
							{product.parts && product.parts.map((part) => <ProductCreateReviewItem>{part}</ProductCreateReviewItem>)}
						</ProductCreateReviewItems>
					</ProductCreateReviewValue>
				</ProductCreateReviewRow>
				<ProductCreateReviewRow>
					<ProductCreateReviewLabel>Solution Attachments</ProductCreateReviewLabel>
					<ProductCreateReviewValue>
						<Thumbnails>
							{product.attachments.map((attachment) => (
								<Thumbnail attachment={attachment} />
							))}
						</Thumbnails>
					</ProductCreateReviewValue>
				</ProductCreateReviewRow>
			</Inner>
		</ProductCreateReviewWrapper>
	);
};

export default ProductCreateReview;
